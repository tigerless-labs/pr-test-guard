#!/usr/bin/env python3
"""Run PR Test Guard regression fixtures and emit test-quality evidence artifacts.
...
def run_case(case_dir: Path, output_root: Path) -> bool:
    ...
    write_report(
        case_output / "pr_test_guard_report.md",
        case_dir.name,
        claim_doc,
        passed,
        coverage_map,
...
def write_report(
    ...
    lines = [
        f"# PR Test Guard Fixture Report: {case_id}",
        "",
        "This report is generated by the regression-fixture runner. Findings are review support signals, not proof that a PR is correct.",
        "",
        "## Test Result",
        "",
